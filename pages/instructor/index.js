import { CheckCircleOutlined, CloseCircleOutlined } from "@ant-design/icons";
import { Avatar } from "antd";
import axios from "axios";
import Link from "next/link";
import { useEffect, useState } from "react";
import { DEFAULT_COURSE_IMG, MIN_LESSONS_REQUIRED } from "../../common/constants";
import InstructorRoute from "../../components/routes/InstructorRoute";

const API_INSTRUCTOR_COURSES = "/api/instructor-courses";

const InstructorIndex = () => {

    const myStyle = {
        marginTop: "-15px",
        fontSize: "10px",
    }

    const [courses, setCourses] = useState([]);
    useEffect(() => {
        loadCourses();
    }, []);
    const loadCourses = async () => {
        const { data } = await axios.get(API_INSTRUCTOR_COURSES);
        setCourses(data);
    }

    return (
        <InstructorRoute>
            <h1 className="jumbotron text-center square">My Courses</h1>
            {/* <pre>{JSON.stringify(courses, null, 4)}</pre> */}

            {courses && courses.map(course => (
                <>
                    <div className="media p-2 row">

                        <Avatar size={80} src={course.image ? course.image.Location : DEFAULT_COURSE_IMG} className="col-4 p-2" />

                        <div className="media-body pl-2 col-8">
                            <div className="row">
                                <div className="col-11">
                                    {/* <Link href={`/instructor/course/view/id/${course._id}`} className="pointer"> */}
                                    <Link href={`/instructor/course/view/${course.slug}`} className="pointer">
                                        <a className="mt-2 text-primary"><h5 className="pt-2">{course.name}</h5></a>
                                    </Link>
                                    <p style={{marginTop: "-0.3rem"}}>{course.lessons.length} Lessons</p>
                                    {course.lessons.length < MIN_LESSONS_REQUIRED
                                        ? (<p style={myStyle} className="text-warning">At least {MIN_LESSONS_REQUIRED} lessons are required to publish a course</p>)
                                        : course.published
                                            ? (<p style={myStyle} className="text-sucess">Your course is live in the website</p>)
                                            : (<p style={myStyle} className="text-sucess">Your course is ready to be published</p>)
                                    }
                                </div>
                                <div className="col-1 mt-3 text-center">
                                    {course.published
                                        ? (<div><CheckCircleOutlined className="h5 pointer text-success" /></div>)
                                        : (<div><CloseCircleOutlined className="h5 pointer text-warning" /></div>)
                                    }
                                </div>
                            </div>
                        </div>


                    </div>
                </>
            ))}
        </InstructorRoute>
    );
}

export default InstructorIndex;